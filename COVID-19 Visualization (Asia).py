...
# # Visualising COVID-19 in Asia

# Visualising the impact of COVID-19 on the Asian nations on the basis of population, hospital beds available per 1000, tests done,confirmed cases and death rates 
...
# Reading the csv data file via Github URL and filtering the data based on the continent 'Asia' start.
...
covid19_data_frame = covid19_data_frame.loc[
    covid19_data_frame['continent'] == 'Asia']  # Filter out data based on Asia continent.
...
countries_in_asia = covid19_data_frame['location'].unique().tolist()
...
color_dict = {countries_in_asia[index]: color_list[index]
              for index in range(len(countries_in_asia))}
...
# creating a data frame from the actual asia data frame
...
c = []
for i in range(0, len(countries_in_asia)):
    c.append(i)

# Allocating the countries unique numbers
lookup = dict(zip(countries_in_asia, c))
num = []
...
fig2 = go.Figure(data=go.Parcoords(
    line=dict(color=num,
              ...
              showscale=False,
              cmin=0,
              cmax=len(countries_in_asia)),
    dimensions=list([
        dict(range=[0, len(countries_in_asia)],
             tickvals=c, ticktext=countries_in_asia,
